...
fn the_show() {
    ...
    assert_eq!(show.description.as_deref(), Some("Jede Woche Politik aus Berlin."), "markup does not survive");
    assert_eq!(show.website.as_deref(), Some("https://beispiel.example"), "atom:link is not the website");
...
fn an_episode_with_everything() {
    ...
    assert_eq!(episode.title, "BS 012 · Haushalt & Rente", "titles escaped twice are decoded twice");
    assert_eq!(episode.published.map(|date| date.to_rfc3339()).as_deref(), Some("2026-09-19T10:33:00+00:00"));
    ...
    assert_eq!(episode.chapters.len(), 2, "a chapter without a readable time is dropped");
    assert_eq!(episode.chapters[1].start_ms, 400_000);
    assert_eq!(episode.chapters[1].url.as_deref(), Some("https://beispiel.example/haushalt"));
    assert_eq!(episode.chapters[0].source, ChapterSource::Feed);
    assert_eq!(episode.chapters_url.as_deref(), Some("https://beispiel.example/012/chapters.json"));
    ...
    assert_eq!(notes.links, vec!["https://beispiel.example/live"], "the full notes win over the teaser");
...
fn html_entities_do_not_break_the_feed() {
    ...
    assert_eq!(episode.notes_html.as_deref(), Some("Fragen & Antworten, R&D inklusive."));
